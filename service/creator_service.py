...
from orchestrator.messages import CreatorRequest as OrchestratorCreatorRequest
...
class CreatorService:
    ...
    async def handle_creator(
        ...
        if input_type == "voice" and voice_data and self._voice:
            logger.info(
                "creator_service:voice_input:processing",
            ...
            try:
                ...
                logger.info(
                    "creator_service:voice_input:success",
                    extra={
                        "correlation_id": correlation_id,
                ...
                logger.error(
                    "creator_service:voice_input:error",
                    extra={"correlation_id": correlation_id, "error": str(e)},
                    exc_info=True,
                )
                raise

        orch_req = OrchestratorCreatorRequest(
            user_id=user_id,
            language=language,
        ...
        result = await self._orchestrator.handle_creator(orch_req, correlation_id)
        ...
        if input_type == "voice" and self._voice and self._voice._tts_enabled:
            logger.info(
                "creator_service:voice_response:generating",
                extra={"correlation_id": correlation_id},
            )
            try:
                voice_url = await self._voice.generate_voice_response(
                    text=result.response_text,
                    conversation_id=f"creator_{user_id}",
                    message_id=agent_message_id,
                )
                output_type = "voice"
                logger.info(
                    "creator_service:voice_response:success",
                    extra={
                        "correlation_id": correlation_id,
                        "voice_url": voice_url,
                    },
                ...
                logger.error(
                    "creator_service:voice_response:error",
                    extra={"correlation_id": correlation_id, "error": str(e)},
                    exc_info=True,
                )
